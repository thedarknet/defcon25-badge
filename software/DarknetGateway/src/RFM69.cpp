...
RFM69::RFM69(bool isRFM69HW, uint8_t interruptID) {
	_mode = RF69_MODE_STANDBY;
	_interruptID = interruptID;
...
//CMDC0DE
	...
	Hardware::get().getSPI1()->masterListen(); //sets CS to high
...
void RFM69::sendFrame(RFM69::RadioAddrType toAddress, const void* buffer, uint8_t bufferSize, bool requestACK,
	...
	// write to FIFO
	select();
	Hardware::get().getSPI1()->transfer(REG_FIFO | 0x80);
	Hardware::get().getSPI1()->transfer(bufferSize + 5);
	Hardware::get().getSPI1()->transfer((toAddress & 0xFF00) >> 8);
	Hardware::get().getSPI1()->transfer((toAddress & 0xFF));
	Hardware::get().getSPI1()->transfer((_address & 0xFF00) >> 8);
	Hardware::get().getSPI1()->transfer((_address & 0xFF));
	Hardware::get().getSPI1()->transfer(CTLbyte);

	for (uint8_t i = 0; i < bufferSize; i++)
		Hardware::get().getSPI1()->transfer(((uint8_t*) buffer)[i]);
...
void RFM69::interruptHandler() {
	...
	if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY)) {
		...
		setMode(RF69_MODE_STANDBY);
		select();
		Hardware::get().getSPI1()->transfer(REG_FIFO & 0x7F);
		PAYLOADLEN = Hardware::get().getSPI1()->transfer(0);
		PAYLOADLEN = PAYLOADLEN > 66 ? 66 : PAYLOADLEN; // precaution
		TARGETID = Hardware::get().getSPI1()->transfer(0) << 8;
		TARGETID |= Hardware::get().getSPI1()->transfer(0);
		...
		SENDERID = Hardware::get().getSPI1()->transfer(0) << 8;
		SENDERID |= Hardware::get().getSPI1()->transfer(0);
		uint8_t CTLbyte = Hardware::get().getSPI1()->transfer(0);
		...
		for (uint8_t i = 0; i < DATALEN; i++) {
			DATA[i] = Hardware::get().getSPI1()->transfer(0);
		}
...
void noInterrupts() {
	...
	Hardware::get().disableIRQ();
}

void interrupts() {
	//HAL_NVIC_EnableIRQ(EXTI0_IRQn);
	Hardware::get().enableIRQ();
}
...
void RFM69::encrypt(const char* key) {
	...
	if (key != 0) {
		select();
		Hardware::get().getSPI1()->transfer(REG_AESKEY1 | 0x80);
		for (uint8_t i = 0; i < 16; i++)
			Hardware::get().getSPI1()->transfer(key[i]);
		unselect();
	}
...
uint8_t RFM69::readReg(uint8_t addr) {
	select();
	Hardware::get().getSPI1()->transfer(addr & 0x7F);
	uint8_t regval = Hardware::get().getSPI1()->transfer(0);
	unselect();
	return regval;
}
...
void RFM69::writeReg(uint8_t addr, uint8_t value) {
	select();
	Hardware::get().getSPI1()->transfer(addr | 0x80);
	Hardware::get().getSPI1()->transfer(value);
	unselect();
}

// select the RFM69 transceiver (save SPI settings, set CS low)
void RFM69::select() {
	//Hardware::get().noInterrupts();
	Hardware::get().disableIRQ();
	Hardware::get().getSPI1()->select();
	//noInterrupts();
...
void RFM69::readAllRegs() {
	...
	for (uint8_t regAddr = 1; regAddr <= 0x4F; regAddr++) {
		select();
		Hardware::get().getSPI1()->transfer(regAddr & 0x7F); // send address + r/w bit
		regVal = Hardware::get().getSPI1()->transfer(0);
		unselect();
		...
		switch ( regAddr )
		{
			...
			case 0x4 : {
				select();
				Hardware::get().getSPI1()->transfer(REG_PACKETCONFIG1 & 0x7F); // send address + r/w bit
				uint8_t checkConfig = Hardware::get().getSPI1()->transfer(0);
				unselect();
				if((checkConfig & 0b100000)!=0) {
					bitRate |= regVal;
					SerialPrint ( "Bit Rate (Chip Rate when Manchester encoding is enabled)\nBitRate : ");
					unsigned long val = 32UL * 1000UL * 1000UL / bitRate;
					SerialPrint( "%lu\n", val );
				} else {
					SerialPrint( "Manchester encoding is turned off: no configurable bit rate");
				}
				break;
			}
			...
			case 0x37: {
				if(regVal & 0x80) {
					SerialPrint( "Packet format: Fixed Length");
				} else {
					SerialPrint( "Packet format: variable lendth");
				}
				SerialPrint("\n");
				uint8_t val = regVal & 0b01100000;
				val = val >> 4;
				if(val==0) {
					SerialPrint( "No encoding");
				} else if (val==1) {
					SerialPrint( "Manchester encoding");
				} else if(val==2) {
					SerialPrint( "Whitening");
				}
				SerialPrint("\n");
				if(regVal & 0b10000) {
					SerialPrint( "CRC ON");
