...
ErrorType AddressState::onInit(RunContext &rc) {
	setNext4Items(0, rc);
...
void AddressState::setNext4Items(uint16_t startAt, RunContext &rc) {
	uint8_t num = rc.getContactStore().getSettings().getNumContacts();
	for (uint16_t i = startAt, j = 0; j < (4); i++, j++) {
		if (i < num) {
			if (rc.getContactStore().getContactAt(i, CurrentContactList[j])) {
				Items[j].id = CurrentContactList[j].getUniqueID();
				Items[j].text = CurrentContactList[j].getAgentName();
			}
		} else if (i == num && rc.getContactStore().getMyInfo().isUberBadge()) {
			//add broadcast
...
ReturnStateContext AddressState::onRun(RunContext &rc) {
