...
ReturnStateContext Gateway::onRun(RunContext &rc) {
	...
	case REQUEST_REP: {
		...
		if (rc.getRadio().sendWithRetry(DCDarkNetApp::GATEWAY_RADIO_ID, &grm, sizeof(grm), 3, 40)) {
			InternalState = RECEIVING;
		} else {
			nextState = StateFactory::getDisplayMessageState(StateFactory::getMenuState(), "No response from Gateway",
				3000);
		}
